#!/usr/bin/env python3

from pwn import *
from hashlib import sha256
from Crypto.Cipher import DES
from Crypto.Util.number import *

l = 16

IP = "01.cr.yp.toc.tf"
PORT = 37113

SBOX = [
	
	]

def shaim(msg):
	nbit, hmsg = 64, msg.hex()
	hmsg += 'f' + hex(len(msg.hex())*4)[2:]
	hmsg += (nbit - (4*len(hmsg) % nbit)) // 4 * 'f'
	H, SBOX_M  = [], ''
	for i in range (0, len(hmsg) - 1, 2):
		tmp = hex(SBOX[int(hmsg[i:i+2], 16)])[2:]
		tmp = '0' + tmp if len(tmp) % 2 else tmp
		SBOX_M += tmp
	hmsg = SBOX_M  
	l = nbit // 4     
	H.append((int(hmsg[0:l], 16)))
	for i in range(1, len(hmsg) // l):
		plain = long_to_bytes(int(hmsg[i*l:(i+1)*l], 16))
		key = long_to_bytes(H[i-1])
		_DES = DES.new(key = key, mode = DES.MODE_OFB, IV = key)
		H.append(bytes_to_long(_DES.encrypt(plain)) ^ bytes_to_long(key))
	dgst = sha256(long_to_bytes(H[-1])).hexdigest()
	return dgst, H, hmsg

def randstr(l):
	rstr = [(string.printable[:62] + '_')[getRandomRange(0, 62)] for _ in range(l)]
	return ''.join(rstr).encode('utf-8')

def encrypt_ecb(pt, key):
    key = long_to_bytes(int(key, 16))
    _DES = DES.new(key = key, mode = DES.MODE_ECB)
    ct = bytes_to_long(_DES.encrypt(long_to_bytes(int(pt, 16))))
    return ct

def inv_sbox(hmsg):
    SBOX_M = ''
    for i in range (0, len(hmsg) - 1, 2):
    	tmp = hex(SBOX.index(int(hmsg[i:i+2], 16)))[2:]
    	tmp = '0' + tmp if len(tmp) % 2 else tmp
    	SBOX_M += tmp
    return SBOX_M

def find_collision(x, y, z):
    while True:
        hmsg_blocks = [randstr(8).hex()]
        H = [int(hmsg_blocks[0], 16)]
        tmp = x ^ encrypt_ecb(hmsg_blocks[0], hmsg_blocks[0]) ^ H[0]
        hmsg_blocks += [hex(tmp)[2:].zfill(16)] + y
        msg = bytes.fromhex(inv_sbox(''.join(hmsg_blocks)))[:z]
        try:
            if (b'\n' not in msg) and (msg.decode().encode('utf-8') == msg):
                return msg
        except:
            continue

io = remote(IP, PORT)
io.sendlineafter("[Q]uit\n", 'g')

io.recvuntil("msg = ")
msg_0 = eval(io.recvline())

dgst_0, H_0, hmsg_0 = shaim(msg_0)
hmsg_0_blocks = [hmsg_0[i * l: (i + 1) * 16] for i in range(len(hmsg_0) // l)]

msg_1 = find_collision(H_0[1], hmsg_0_blocks[2:], len(msg_0))
dgst_1, _, _ = shaim(msg_1)

assert dgst_0 == dgst_1

io.sendlineafter("[Q]uit\n", 's')
io.sendlineafter("hash: ", msg_1)

io.interactive()

# CCTF{A9aiN____DES____N0T_w3AkNEs5!!}