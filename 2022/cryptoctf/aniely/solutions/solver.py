...
tmp = bytes(a ^ b for a, b in zip(key, enc))
for rand in tqdm(range(65536)):
    rand = rand.to_bytes(2, 'big') * 16
    FLAG = bytes(a ^ b ^ c for a, b, c in zip(tmp, aniely_stream(key), rand))
    if b"CCTF{" in FLAG:
        print(FLAG)
        break
